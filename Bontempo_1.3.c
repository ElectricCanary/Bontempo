...
PROGMEM prog_uint16_t tempo[] ={			//wiper position to ms tempo conversion chart for PT2399, calibrated with PWM at 300 (no modulation)
...
int main(void)
{
	...
	WAVEPORT |= (1<<WAVEPIN);	//Connecting Pull-Up Resistor of toggle pins
	DIVPORT |= (1<<DIVPIN);
    ...
    while (1) 
    {
		...
		speed = 100;//255 - speedvalue; //updating mod speed

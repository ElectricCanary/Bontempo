...
int main(void)
{
    ...
    while(1)
    {
		...
		if(previousmult != mult)					//If toggle changes, update the digital pot value (if in tap mode)
		{
			if (debounce()==1)						//Alternative divisions of button pressed
			{
				if(mult == 1)
				{
					altmult = 0.333333;
			...
			if (tap == 1)							//if in tap mode transfert the new delay value to digital pot
			{
				if (alt == 1)
				{
					data = findClosest(tempo,n,round(mstempo*altmult));
					databyte = data;
			...
			previousmult = mult;					//reinitialize previous pot value
		}
		...
		if(debounce() == 1 && laststate == 0 && nbpress != 0) //second, third press...
		{
			...
			if (alt==1)	//if alternative division on
			{
				data = findClosest(tempo, n, round(mstempo*altmult));
				databyte = data;
			...
			if (alt==0)	//if alternative division off
			{
				data = findClosest(tempo, n, round(mstempo*mult));
			databyte = data;
			SPI_MasterTransmit(commandbyte,databyte);			//Transmits 16bit word
...
ISR(ADC_vect)								// Interrupts when conversion is finished
{
	...
	switch(ADMUX)							//Switches between the pot and toggle ADC
	{
		...
		{
			mult = 0.5;					//Anything in between : dotted eighth
		}

 ...
 * Author : Antoine Ricoux
 ...
 4: Double Time Pin (if RESET is disabled)
 5: SPI Chip Select
 ...
 Low Fuse = 0xd2	High Fuse = 0x5c !!!CAREFUL!!! RESET disabled, double time pin enabled
...
uint8_t doubletimecheck(void)	//Check double time pin status
{
	/*if (bit_is_clear(DOUBLESFR,DOUBLEBV))
	{
		return(1);
	}
...
{	
	...
	uint16_t mstempo = eeprom_read_word((uint16_t*)64);		//tempo for toggling LED (not influenced by tempo div)
	uint8_t delaymin = 51;
	...
	uint8_t tap = eeprom_read_byte((uint8_t*)32);		//tap controlled (1) or pot controlled (0)
	uint8_t tapping = 0;	//leds follow tap (1) or follow tempo/div (0)
	...
	uint8_t i = 1; //is used for delaying the time button read, (the eeprom time save is not taken into account otherwise) 
    ...
    while (1) 
    {
		...
		if (wavetype == 2)	//Triangle
		{
			...
			if (inc > 80 && inc <= 173)
			{
				currentinc = pgm_read_word_near(triangle + inc - 81);
			}
			if (inc > 173 && inc < 255)
			{
		...
		if (tap != 1 || abs(previoustimevalue-timevalue) >= 13)	//if pot move of more than 5%, changing to pot control
		{
			...
			previoustimevalue = timevalue;
			tap = 0;
			eeprom_update_byte((uint8_t*)32, 0);
		}
		...
		if (abs(previousdiv - divtogglevalue) > 50)		//if first time or if div toggle changed position : update div
		{
			...
			if (doubletimecheck() == 1)		//if doubletime mode is active: divide any div per 2
			{
				divmult = divmult / 2;
			}
		...
		if (msturns > ((delaymax/divmult)+800)) //if too long between taps  : resets
		{
			...
			nbtap = 0;
			tapping = 0;
			eeprom_update_word((uint16_t*)64, mstempo);

...
int main(void)
{
    ...
    while(1)
    {
		...
		{
			data = (((mstempo*mult)-MINDELAY)/((MAXDELAY-MINDELAY)/100))*2.55;
			databyte = (int)data;
		...
		if (ledturns > mstempo)				//Toggles Led every downbeat
		{
			LEDPORT |= (1 << LEDPIN);
			_delay_ms(5);
			LEDPORT &= ~(1<<LEDPIN);
...
ISR(ADC_vect)								// Interrupts when conversion is finished
{
	...
	switch(ADMUX)							//Switches between the pot and toggle ADC
	{
		...
		if(togglevalue <= 84)				//if pin low : eighth
		{
			mult = 0.5;
		}
		if(togglevalue >= 171)				//if pin high : fourth
		{
			mult = 1;
		}
		else

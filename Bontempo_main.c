 ...
 * Author : Antoine Ricoux
 ...
 Low Fuse = 0xd2	High Fuse = 0x5c !!!CAREFUL!!! RESET disabled, double time pin enabled (need to change commentary in "doubletimecheck")
...
uint8_t doubletimecheck(void)	//Check double time pin status	!!!!change the commentary if RESET disable!!!!
{
...
{	
    ...
    while (1) 
    {
		...
		if (wavetype == 2 && (inc%50) == 0)
		{
			currentinc = rand() / (RAND_MAX / 401);				//random
		}
	
		if (wavetype == 3)	//Triangle
		{
			if (inc <= 80)
			{
					...
					if (wavetype == 4)	//Sawtooth
		{
			...
			if (inc > 162)
			{
				currentinc = pgm_read_word_near(saw + inc - 163);			//Sawtooth
					...
					if (wavetype == 5)	//Reverse Sawtooth
		{
			if (inc <= 92)
			{
